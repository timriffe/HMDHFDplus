...
#' @title internal function for modifying freshly read HMD data in its standard form
#'
#' @description called by \code{readHMD()} and \code{readHMDweb()}.
#' We assume there are no factors in the given data.frame and that it has been
#' read in from the raw text files using something like:
#' \code{ read.table(file = filepath, header = TRUE, skip = 2,
...
#' @param filepath just to check if these are population counts from the name.
#'
#' @return DF same data.frame, modified so that columns are of a useful class.
#' If there were open age categories, such as \code{"-"} or \code{"+"},
#' this information is stored in a new dummy column called \code{OpenInterval}.
#'
#' @details This parse routine is based on the subjective opinions of the author...
#'
#' @export
HMDparse <- function(DF, filepath){
  ...
  ## Population.txt / Population5.txt special handling
  if (grepl("pop", tolower(filepath))) {
    
    # helper that applies the existing logic to a *single* population series
    split_pop <- function(DF_pop) {
      # what years do we have (within this PopName, if present)?
      all.years <- sort(unique(age2int(DF_pop$Year)))
      
      # make indicators:
      Pluses  <- grepl(pattern = "\\+", DF_pop$Year)
      Minuses <- grepl(pattern = "\\-", DF_pop$Year)
      
      # split out DF into two parts
      Jan1i  <- DF_pop$Year %in% as.character(all.years[-length(all.years)]) | Pluses
      Dec31i <- DF_pop$Year %in% as.character(all.years[-1])                 | Minuses
      
      Jan1   <- DF_pop[Jan1i, , drop = FALSE]
      Dec31  <- DF_pop[Dec31i, , drop = FALSE]
      ...
      colnames(Jan1)[cols1]  <- paste0(colnames(Jan1)[cols1],  1L)
      colnames(Dec31)[cols2] <- paste0(colnames(Dec31)[cols2], 2L)
      
      DF_out <- cbind(Jan1, Dec31[, cols2, drop = FALSE])
      
      # finally reorganize columns:
      orgi   <- grepl("male",  tolower(colnames(DF_out))) |
        grepl("total", tolower(colnames(DF_out)))
      DF_out <- cbind(DF_out[, !orgi, drop = FALSE],
                      DF_out[,  orgi, drop = FALSE])
      
      DF_out
    }
    
    if ("PopName" %in% colnames(DF)) {
      ## stacked Population.txt with PopName column:
      DF_list <- split(DF, DF$PopName)
      DF_list <- lapply(DF_list, split_pop)
      DF      <- do.call(rbind, DF_list)
      rownames(DF) <- NULL
    } else {
      ## old behavior: single-country Population.txt
      DF <- split_pop(DF)
    }
  }
  
  ## Year / Cohort parsing (unchanged – safe even if Year already integer)
  if (any(grepl("year", tolower(colnames(DF))))) {
    DF$Year <- age2int(DF$Year)
  }
  if (any(grepl("cohort", tolower(colnames(DF))))) {
